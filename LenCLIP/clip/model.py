...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, require_all_fts=False):
        # Frozen weights still propagate gradients to learnable prompt/mask inputs.
        # Explicitly run every block; token count must not select image/text mode.
        x_all, attn_weights = [], []
        for block in self.resblocks:
            if getattr(self, "gradient_checkpointing", False) and torch.is_grad_enabled() and x.requires_grad:
                x, attn = checkpoint(block, x, use_reentrant=False)
            else:
                x, attn = block(x)
            if require_all_fts:
                x_all.append(x)
                attn_weights.append(attn)
        return (x_all if require_all_fts else x), attn_weights
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W, require_all_fts=False):
        ...
        position = upsample_pos_emb(self.positional_embedding, (H//self.patch_size, W//self.patch_size))
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text):
        tokens, _ = self.encode_image(image, *image.shape[-2:])
        image_features = self.visual.ln_post(tokens[0]) @ self.visual.proj
